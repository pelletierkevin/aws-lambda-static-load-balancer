...
def retrieve_privateIP_from_public(lookup_result_set):
    
    Filters=[{'Name': 'association.public-ip', 'Values': ['35.166.211.118']}]

    private_ip_result_list = []
    for public_ip in lookup_result_set:
        Filters[0]['Values'] = [public_ip]
        privateAddress = ec2client.describe_network_interfaces(Filters=Filters)['NetworkInterfaces'][0]['PrivateIpAddress']
        print "PRIVATE IP FOUND : " + privateAddress
        private_ip_result_list.append(privateAddress)
        
    return set(private_ip_result_list)
...
def lambda_handler(event, context):
    ...
    # CONVERT PUBLIC IP TO PRIVATE IP FROM AWS LIBRARY
    privateIPs = retrieve_privateIP_from_public(regular_record_set)
    print "INFO: PRIVATE IPs detected:", privateIPs
    print "INFO: Number of PRIVATE IPs detected : ", len(privateIPs)
    ...
    new_active_ip_dict["IPList"] = list(privateIPs)
    new_active_ip_dict["IPCount"] = len(privateIPs)
